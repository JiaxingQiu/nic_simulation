plot_ls <- list()
for(rn in c("lm","lr")){
  agg_df_org <- agg_df_ls[[rn]] %>% filter(na_rate == 0) %>% as.data.frame()
  
  # group conditions to low, median, high clustering
  # 10
  agg_df <- agg_df_org[which(agg_df_org$n_cluster==10),]
  ...
  agg_df1 <- agg_df[!is.na(agg_df$cluster_strength),]
  # 50
  agg_df <- agg_df_org[which(agg_df_org$n_cluster==50),]
  agg_df$cluster_strength <- NA
  agg_df$cluster_strength[which(agg_df$n_obs_per_cluster==25 &
                                  agg_df$sigma_rdm_fix_ratio==0.5 &
                                  agg_df$ar1_phi == 0 )] <- "weak"
  ...
  agg_df$cluster_strength[which(agg_df$n_obs_per_cluster==100 &
                                  agg_df$sigma_rdm_fix_ratio==10 &
                                  agg_df$ar1_phi == 0.8 )] <- "strong"
  agg_df0 <- agg_df[!is.na(agg_df$cluster_strength),]
  ...
  plot_df$n_cluster <- factor(plot_df$n_cluster, levels=c(10,50) )
  levels(plot_df$n_cluster) <- c("10 clusters", "50 clusters")
  ...
  plot_ls[[rn]] <- ggplot(data = plot_df, aes(x = n_ttl_betas, y = ic_diff, color = ic_type)) + 
    ...
    # coord_trans(y = "sqrt") +
    facet_wrap(cluster_strength ~ n_cluster, ncol=2, nrow=3, scales="free_x") + 
