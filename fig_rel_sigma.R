...
for(rn in c("lm","lr")){ 
  ...
  plot_df <- plot_df %>% filter(na_rate == na) %>% as.data.frame()
  ...
  ggplot(data = plot_df, aes(x = ic_diff, fill = as.factor(sigma_rdm_fix_ratio) )) + 
    geom_histogram(position = "dodge") +
    facet_wrap(~ic_type+sigma_rdm_fix_ratio, ncol=4, nrow=3)
