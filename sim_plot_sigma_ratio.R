...
for(rn in c("lm","lr")){
  ...
  plot_df$sigma_rdm_fix_ratio <- factor(plot_df$sigma_rdm_fix_ratio, levels=sort(unique(plot_df$sigma_rdm_fix_ratio)) )
  ...
  plot_ls[[rn]] <- ggplot(data = plot_df, aes(x = n_ttl_betas, y = ic_diff, color = ic_type)) + 
    ...
    # coord_trans(y = "sqrt") +
    facet_wrap(~ sigma_rdm_fix_ratio, nrow=1, scales="free_x") + 
