for(looby in c("loopred","loodev") ){
  plot_ls <- list()
  for(rn in c("lr", "lm")){ 
    plot_df <- pivot_longer(agg_df_ls[[rn]], 
                            cols = starts_with(looby), 
                            names_to = "model", 
                            values_to = "loopred")
    ...
    plot_df$n_obs_per_cluster <- factor(plot_df$n_obs_per_cluster, levels=c("5 obs/cluster",
                                                                            ...
                                                                            "50 obs/cluster",
                                                                            "80 obs/cluster") )
    plot_df$model <- ifelse(plot_df$model%in%c("loopred0","loodev0"),"GLME", "GLM")
    ...
    plot_ls[[rn]] <- ggplot(data = plot_df, aes(x = n_ttl_betas, y = loopred, color = model)) + 
      geom_point(size=1) +
      ...
      labs(title = ifelse(rn=="lr", "Binomial", "Gaussian"),
           ...
           y = ifelse(rn=="lr", ifelse(looby=="loodev", "looDeviance", "looAUC"), ifelse(looby=="loodev", "looDeviance", "looMSE") ),
           color = "Model",
           linetype="Missing") + 
      theme_bw()
    # if(rn=="lr" & looby=="loopred"){
    #   plot_ls[[rn]] <- ggplot(data = plot_df, aes(x = n_ttl_betas, y = loopred, color = model)) +
    #     geom_point(size=1) +
    #     geom_line(mapping = aes(linetype=na_rate_factor))+
    #     scale_linetype_manual(values = c("solid", "dashed", "dotdash", "dotted")) +
  ...
  p %>% ggsave(filename=paste0("./res/pred_lm_vs_lme_",looby,".png"), width = 15, height = 7, bg="white")
}
